	...
	public static void main(String[] args) {
		...
		for (int rodada = 0; rodada <=102;rodada++)
		{
			...
			for (int time = 0; time <=3;time++) {
				...
				{
					tabela[time][rodada+2] = "3"; // joga a pontuação no time no numero da rodada
					tabela[time][1] =Integer.toString((Integer.parseInt(tabela[time][1])+3)); // terceira coluna [2] é o primera rodada e assim segue
					
				}
				if (opc == '2') // Time empatou
				{
					tabela[time][rodada+2] = "1"; // joga a pontuação no time no numero da rodada
					tabela[time][1] =Integer.toString((Integer.parseInt(tabela[time][1])+1)); // terceira coluna [2] é o primera rodada e assim segue
				}
		...
		if (leitura == 'S') {
			...
			for (int time = 0; time <=3;time++) {
				for (int rodada = 0; rodada <=102;rodada++) {
					if (tabela[0][rodada] == null) break;
					if(rodada == 0) System.out.printf("%s\t",tabela[time][rodada]);
					else if(rodada == 1) System.out.printf("  %s\t",tabela[time][rodada]);
					else System.out.printf("   %s\t",tabela[time][rodada]);
